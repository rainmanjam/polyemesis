 ...
 *
 * The 23 rendered documentation pages build to dist/docs/<slug>.html, and each
 * check below fails differently on them if this is not recursive. The one that
 * matters most here is the hand-rolled-<pre> ban: rendering docs/*.md is the
 * change that puts 114 more code blocks on this site, and a flat listing would
 * have exempted every one of them from the guard that exists to catch exactly
 * that -- reporting a pass while not looking. The amber reservation, the
 * internal-link check and the _headers rule go the same way.
 *
