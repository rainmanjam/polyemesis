...
type coverageLedger struct {
	...
	// CitedIssues is every issue number any excuse or deferral names,
	// regenerated by -update-coverage. It is a FORM check and nothing more: it
	// catches a typo, and adding a citation becomes a visible artifact diff.
	// The liveness half -- "does a reachable commit announce closing it" -- is
	// TestNoLedgerCitationNamesAnIssueACommitClosed, and it runs git.
...
// driveExcuse issues the excuse's claim as a real request. "ANY" is driven as
// GET, since that is the method every ANY-registered mount answers in this
// router; the per-pair rules below cover the rest of the methods.
func driveExcuse(t *testing.T, h http.Handler, read, key string) excuseObservation {
	t.Helper()
...
func assertExcusesDischargeByRunning(t *testing.T, h http.Handler, read string) {
	t.Helper()
	...
	for _, key := range keys {
		...
		if ex.Want.Status == 0 && !ex.Want.AnonMatchesRead {
			t.Errorf("the excuse for %s carries a Want that claims nothing: no status and "+
				...
				"fail, or it is the prose it replaced with a struct around it.", key)
			continue
		}
		obs := driveExcuse(t, h, read, key)
		if ex.Want.Status != 0 && obs.ReadStatus != ex.Want.Status {
			t.Errorf("EXCUSE PREMISE FALSE. %s: the excuse claims a %s principal receives "+
				...
				"otherwise correct the Want to the status the router actually imposes.",
				key, ex.Want.As, ex.Want.Status, obs.Method, concretePath(
					strings.TrimPrefix(key, obs.Method+" ")), obs.ReadStatus,
				len(obs.ReadBody), truncateForFailure(obs.ReadBody))
		}
		if ex.Want.AnonMatchesRead && !obs.Identical {
			t.Errorf("EXCUSE PREMISE FALSE. %s: the excuse claims an anonymous stranger and "+
				...
				key, truncateForFailure(obs.ReadBody), truncateForFailure(obs.AnonBody))
		}
		// (c) THE RULE THAT CANNOT BE EXCUSED AWAY.
		if obs.ReadStatus/100 == 2 && strings.TrimSpace(obs.ReadBody) != "" && !ex.Want.AnonMatchesRead {
			t.Errorf("EXCUSED ROUTE ANSWERS A READ TOKEN. %s returned %d with %d bytes to a "+
				...
				"scanned.\nbody: %s",
				key, obs.ReadStatus, len(obs.ReadBody),
				concretePath(strings.TrimPrefix(key, obs.Method+" ")),
...
func TestLedgerPreflight(t *testing.T) {
	// The liveness marker. The Makefile asserts that this line still appears
	// under `go test ./internal/api -run XXXNoSuchTest -v`, so the preflight's
	// own existence is proven by RUNNING something that fails if it stopped --
	// which is the rule this whole PR is about, applied to the guard itself.
	...
	// 1. THE DISCHARGE RULE. #164, #166, and the excuse half of #163.
	assertExcusesDischargeByRunning(t, h, read)
	...
	// 8. CITATIONS. Form here, liveness in the git scan below.
...
func assertCitationsAreWellFormed(t *testing.T, want coverageLedger) {
	...
	for _, id := range citedIssues() {
		...
		if !committed[id] {
			t.Errorf("the ledger cites %s, which is not in citedIssues in %s. Regenerate "+
				"with -update-coverage so a new citation lands as a visible diff rather "+
				"than as a string somebody typed into a Go file.", id, coveragePath)
		}
	}
}
...
// assertSweepVerdictsEqual is the partition's half of the equality rule. It
// compares the CLASS, not just the path -- the same lesson assertRouteSetsEqual
// learned when #171 moved eight routes from excused to denied and the ledger
// only noticed because it compared verdicts.
//
// A route sliding from differential to invariant fails HERE by name, before the
// floor check gets to it, so the message says which route stopped carrying a
// credential rather than only that one did.
	...
	return []coverageDefer{
		...
		},
		{
			...
			WhySafe: "CI sets POLYEMESIS_LEDGER=strict, which runs them from inside the " +
				"preflight as well, so no single -run filter silences both paths.",
