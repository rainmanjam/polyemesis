...
// sourcePlaylist is appended LAST to kinds, and playlistRunning is nested
...
func allSourceChoices() []sourceChoice {
	kinds := []sourceKind{sourceNone, sourcePrimary, sourceBackup, sourceSlate, sourcePlaylist}
					...
					for _, backupLive := range bools {
						for _, backupEnabled := range bools {
							for _, slateEnabled := range bools {
								...
								// ask about it.
								for _, playlistRunning := range bools {
									for _, autoReturn := range bools {
										...
										out = append(out, sourceChoice{
											...
											playlistRunning: playlistRunning,
											grace:           goldenGrace,
...
func goldenRow(c sourceChoice) string {
	...
	return fmt.Sprintf(
		...
		c.backupEnabled, c.slateEnabled, c.playlistRunning, c.autoReturn,
		orNone(kind), reason,
	)
}

// goldenRowBeforeThePlaylist renders a row in the EXACT format the table used
// before sourcePlaylist existed -- no playlistRunning column.
//
...
func TestChooseSourceGoldenIsExhaustive(t *testing.T) {
	...
	// (4->5, twice) and one boolean (playlistRunning), so the growth is
...
// sourcePlaylist existed: 1024 rows, the pre-playout format, generated by the
...
// sourcePlaylist existed. The file carries no header line saying so, and that
...
// not: goldenRow renders every field into one line, so ADDING the playlistRunning
...
func TestAdmittingThePlaylistMovedNoDecisionThatDidNotInvolveIt(t *testing.T) {
	...
	for _, c := range allSourceChoices() {
		...
		if c.playlistRunning || c.cur == sourcePlaylist || c.pinned == sourcePlaylist {
			continue
		}
