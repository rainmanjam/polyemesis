...
| D9 | SonarCloud | **`S2068` fires on 39 findings, all false positives**: 37 translated UI labels, 1 comment *about* not hard-coding passwords, 1 line of SRT help text | Low | Fixed in `sonar-project.properties`, scoped per rule and per path. The "will fail the next locale PR" part was **unsupported** — the gate is new-code-only and passes today with all 39 present. The real reason is signal: a rule wrong about all 39 is a rule nobody reads |
