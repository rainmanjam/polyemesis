...
| **10** | [Destination settings & metadata](DESTINATION-SETTINGS.md) | **14–19 d** in 4 parts | ✅ **A–D shipped** | Three metadata fields against a documented ten-plus, two of which are compliance items. Shipped 2026-07-29 bar the items under [what remains](DESTINATION-SETTINGS.md#what-remains) — Facebook's metadata surface, and frame-accurate go-live, which turned out not to be metadata at all |
...
| — | [Light mode, and the card](THEME-AND-UI.md) | — | **planned v0.3.0** | Light mode is an argument with a stated design decision, not a missing feature: the signal palette exists because dark surfaces make trouble readable across the room. Recommends the PUBLIC PLAYER first, console stays dark. References captured 2026-08-04 |
