...
| `398-e-probe-window.sh` | The real command carries `-analyzeduration 15000000`. Does a destination stopped inside that window write only a header? And how long before it writes anything? | **No header-only file**: stopped with SIGTERM at 8s it holds 1.9 MB and 8.6s of media. **But not because probing had ended.** The signal interrupts the probe and FFmpeg writes out what it buffered; left alone, the first output arrives at 15.5s -- the whole window -- because the ffmpeg CLI forces the demuxer's `scan_all_pmts` to 1, which stops it ever declaring its header complete. With `-scan_all_pmts 0` it arrives at 3.5s (8.1.2). An earlier version of this row read the stopped-at-8s file as "FFmpeg starts writing long before the ceiling", which was wrong, and every destination start paid the 15s until it was corrected. |
