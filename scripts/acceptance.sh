...
# poka-yoke: the source is a real ffmpeg publisher and the driver below runs
# via `go run` -- see lib-preflight.sh.
...
go run "$SCRIPTS/acceptance_driver.go" "$SCRIPTS/driverhelpers.go" "$PORT" "$RELAY" 2>&1 | sed 's/^/  /' 
