...
kills_so_far() {
  ...
  # `grep -c … || echo 0` emits TWO zeros and every later $(( … )) sees "0\n0"
  # and dies with a syntax error. `|| :` swallows the exit status without adding
  # a second line; grep has already printed the count this function wants.
  [ -f server.log ] || { printf 0; return; }
  grep -c "$KILL_LINE" server.log 2>/dev/null || :
}
